    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Member</th>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Body #</th>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Rating</th>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Trips</th>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Complaints</th>
                    <th class="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-500">Status</th>
                    <th class="px-4 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-slate-500"></th>
                </tr>
            </thead>
                ...
                @foreach ($members as $member)
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="whitespace-nowrap px-4 py-3">
                            ...
                            <div class="text-[11px] text-slate-400">{{ $member->plate_number }}</div>
                        </td>
                        <td class="whitespace-nowrap px-4 py-3 text-sm text-slate-600">{{ $member->body_number ?? '—' }}</td>
                        <td class="whitespace-nowrap px-4 py-3">
                            <span class="text-sm font-bold text-slate-800">{{ $memberAvg ? number_format($memberAvg, 1) : '0.0' }}</span>
                            <div class="flex gap-0.5">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i class="bi {{ $i <= round((float) $memberAvg) ? 'bi-star-fill text-gold' : 'bi-star text-slate-200' }} text-[10px]"></i>
                                @endfor
                            </div>
                        </td>
                        ...
                        <td class="whitespace-nowrap px-4 py-3 text-right">
                            <button type="button" class="tw-btn tw-btn-outline px-3 py-1 text-xs" data-president-member="{{ $member->id }}" data-url="{{ $memberDetailUrl }}/{{ $member->id }}" data-name="{{ $member->user->name }}"><i class="bi bi-eye mr-1"></i> View</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
