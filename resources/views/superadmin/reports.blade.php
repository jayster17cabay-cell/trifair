...
<div class="tw-page-head">
    <div>
        <h1 class="tw-page-title">Operator Performance Reports</h1>
...
<div class="tw-table-wrap">
    <div class="overflow-x-auto">
        <table class="tw-table">
            <thead>
                <tr>
                    <th class="tw-th w-10"></th>
                    <th class="tw-th">#</th>
                    <th class="tw-th">Operator Name</th>
                    ...
                    <th class="tw-th text-center">Total Ratings</th>
                </tr>
            </thead>
            <tbody>
                @forelse ($operators as $operator)
                    <tr class="tw-tr-hover" style="cursor: pointer;" onclick="toggleTrips({{ $operator->id }})">
                        <td class="tw-td text-center">
                            <i class="bi bi-chevron-right" id="icon-{{ $operator->id }}" style="transition: transform 0.2s; color: #94a3b8;"></i>
                        </td>
                        ...
                        <td class="tw-td text-sm">{{ $operator->license_number ?? 'N/A' }}</td>
                        <td class="tw-td">
                            @if ($operator->status === 'active')
                                <span class="tw-badge tw-badge-navy"><i class="bi bi-check-circle"></i>Active</span>
                            @else
                                <span class="tw-badge tw-badge-amber"><i class="bi bi-x-circle"></i>{{ ucfirst($operator->status) }}</span>
                            @endif
                        </td>
                        <td class="tw-td">
                            <div class="flex items-center gap-2">
                                <div class="flex gap-0.5">
                                    ...
                                    @for ($i = 1; $i <= 5; $i++)
                                        @if ($i <= round($avg))
                                            <i class="bi bi-star-fill text-xs text-amber-500"></i>
                                        @else
                                            <i class="bi bi-star text-xs text-slate-300"></i>
                                        @endif
                                    @endfor
                                </div>
                    ...
                    <tr id="trips-{{ $operator->id }}" style="display:none;" data-trips-url="{{ route('superadmin.reports.trips', $operator->id) }}">
...
<script>
    function toggleTrips(id) {
        ...
        const body = row.querySelector('.report-trips-body');
