...
<div class="mb-5 flex flex-wrap gap-2">
    <a href="{{ route('tfrb-officer.complaints', ['filter' => 'pending']) }}" class="{{ $filter === 'pending' ? 'tw-btn tw-btn-sm tw-btn-navy' : 'tw-btn tw-btn-sm tw-btn-outline' }}">
        <i class="bi bi-clock-history"></i> Pending <span class="tw-badge tw-badge-amber ml-1">{{ $pendingCount }}</span>
    </a>
    <a href="{{ route('tfrb-officer.complaints', ['filter' => 'reviewed']) }}" class="{{ $filter === 'reviewed' ? 'tw-btn tw-btn-sm tw-btn-navy' : 'tw-btn tw-btn-sm tw-btn-outline' }}">
        <i class="bi bi-check-circle"></i> Reviewed <span class="tw-badge tw-badge-green ml-1">{{ $reviewedCount }}</span>
    </a>
    <a href="{{ route('tfrb-officer.complaints', ['filter' => 'all']) }}" class="{{ $filter === 'all' ? 'tw-btn tw-btn-sm tw-btn-navy' : 'tw-btn tw-btn-sm tw-btn-outline' }}">
...
@forelse ($complaints as $rating)
    @php $avBg = $avatarColors[$loop->index % count($avatarColors)]; @endphp
    <div class="tw-card mb-3">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-b-2xl bg-slate-50 px-5 py-3">
            ...
            <div class="flex shrink-0 gap-1.5">
                ...
                <form action="{{ route('tfrb-officer.complaints.destroy', $rating) }}" method="POST" onsubmit="return confirm('Delete this complaint?')">
                    @csrf
                    @method('DELETE')
