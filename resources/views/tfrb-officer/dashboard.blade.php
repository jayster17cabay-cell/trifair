...
<div class="relative mb-6 overflow-hidden rounded-3xl bg-gradient-to-br from-navy-700 via-navy-600 to-navy-500 p-6 text-white shadow-soft sm:p-8">
    <div class="pointer-events-none absolute -right-16 -top-24 h-64 w-64 rounded-full bg-[radial-gradient(circle,rgba(16,185,129,0.2)_0%,transparent_70%)]"></div>
    <div class="pointer-events-none absolute -bottom-24 -left-16 h-64 w-64 rounded-full bg-[radial-gradient(circle,rgba(245,166,35,0.16)_0%,transparent_70%)]"></div>
    <div class="relative z-10">
        <p class="text-xs font-bold uppercase tracking-widest text-emerald-400">TFRB Officer</p>
...
<div class="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
    <a href="{{ route('tfrb-officer.operators') }}" class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-soft">
    ...
    <a href="{{ route('tfrb-officer.ratings') }}" class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-soft">
    ...
    <a href="{{ route('tfrb-officer.todas') }}" class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-soft">
...
<div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
    ...
    <a href="{{ route('tfrb-officer.operators') }}" class="tw-card group flex flex-col items-center gap-2.5 p-4 text-center transition hover:-translate-y-0.5 hover:shadow-soft">
        <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-navy-600/10 text-lg text-navy-600 transition group-hover:scale-105"><i class="bi bi-people"></i></div>
...
<div class="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
    @foreach ($todaStats as $toda)
        ...
        <button type="button" onclick="showTodaMembers({{ $toda->id }}, @js($toda->name))" class="tw-card flex cursor-pointer items-center gap-3 p-4 text-left transition hover:-translate-y-0.5 hover:shadow-soft">
            <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-cyan-50 text-lg text-cyan-600">
