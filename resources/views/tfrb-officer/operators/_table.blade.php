    ...
    @forelse ($operators as $operator)
        ...
        <tr class="tw-tr-hover">
            ...
            <td class="tw-td text-center">
                <a href="{{ route('tfrb-officer.operators.qrcode', $operator) }}" class="tw-btn tw-btn-sm tw-btn-outline" title="View QR Code">
            ...
            <td class="tw-td text-right">
                <div class="inline-flex gap-1.5">
                    @if (request('status') === 'pending')
                        <form action="{{ route('tfrb-officer.operators.approve', $operator) }}" method="POST">
                            @csrf
                            @method('PATCH')
                        ...
                        <form action="{{ route('tfrb-officer.operators.reject', $operator) }}" method="POST">
                            @csrf
                            @method('PATCH')
                    ...
                    @else
                        <a href="{{ route('tfrb-officer.operators.edit', $operator) }}" class="tw-btn tw-btn-sm tw-btn-outline" title="Edit">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form action="{{ route('tfrb-officer.operators.destroy', $operator) }}" method="POST" onsubmit="return confirm('Delete this operator? This action cannot be undone.')">
                            @csrf
                            @method('DELETE')
